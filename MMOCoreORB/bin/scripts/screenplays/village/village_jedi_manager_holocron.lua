...
function VillageJediManagerHolocron.useTheHolocron(pSceneObject, pPlayer)
	...
	if CreatureObject(pPlayer):hasSkill("force_title_jedi_rank_02") and not CreatureObject(pPlayer):hasSkill("force_title_jedi_rank_03") and not CreatureObject(pPlayer):villageKnightPrereqsMet("") then	
		...
		sui.setPrompt("You are not yet ready to continue your path padawan. use /findmytrainer to locate your jedi skill trainer.")
		sui.sendTo(pPlayer)
	...
	if CreatureObject(pPlayer):hasSkill("force_rank_dark_master") then
		...
		sui.setPrompt("You sense a disturbance in the force... a dark figure is hunting you, stay alert.")
		sui.sendTo(pPlayer)
	...
	if CreatureObject(pPlayer):hasSkill("force_title_jedi_rank_03") then
		...
		awardSkill(pPlayer, "force_rank_light_master")
					...
					sui.setTitle("FORCE RANKING SYSTEM")
						sui.setPrompt("You have been awarded with enough skill points to learn everything in the game. To learn non jedi skills you have to surrender all jedi skills.")
					sui.sendTo(pPlayer)
		end
			...
			if (pInventory == nil or SceneObject(pInventory):isContainerFullRecursive()) then
				...
				giveItem(pInventory, "object/tangible/wearables/robe/robe_jedi_light_s05.iff", -1)
					...
					sui.sendTo(pPlayer)
								...
								local sui = SuiMessageBox.new("JediTrials", "emptyCallback") -- No callback
					sui.setTitle("FORCE RANKING SYSTEM")
					sui.setPrompt("You have been awarded with 20 skill points. You now have enough skill points to learn all Jedi skills.")
					sui.sendTo(pPlayer)
		end
		...
		awardSkill(pPlayer, "force_rank_light_rank_09")
							local sui = SuiMessageBox.new("JediTrials", "emptyCallback") -- No callback
					sui.setTitle("FORCE RANKING SYSTEM")
					sui.setPrompt("You have been awarded with 20 skill points.")
					sui.sendTo(pPlayer)
		end
